...
const AdminrecruiterDashboard = () => {
  ...
  const fetchRecruiters = async () => {
    try {
      const res = await axios.get(`${API_URL }/admin/recruiters`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this recruiter?")) {
      try {
        await axios.delete(`${API_URL }/admin/recruiters/${id}`);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${API_URL }/admin/recruiters/${editingRecruiter}`, {
        ...formData,
      });
