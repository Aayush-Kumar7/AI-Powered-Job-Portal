...
const ApplicationAdmin = () => {
  ...
  const fetchApplications = async () => {
    try {
      const res = await axios.get(`${API_URL }/applications/applications`); 
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await axios.put(`${API_URL }/applications/applications/${id}`, {
        status: newStatus,
      });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this application?")) {
      try {
        await axios.delete(`${API_URL }/applications/applications/${id}`);
